        ...
        <>
            <div className='relative my-7 md:mt-3'>
                <div className='travigo-container'>
                    ...
                    <div className='grid items-center grid-cols-4 lg:grid-cols-3 md:grid-cols-2'>
                        {placesAPI?.map((val, index) => (
                            <div key={index} className='flex items-center gap-5 sm:gap-3 rounded-lg'>
                                <div className='flex items-center'>
                                    <img src={val.placeImg} 
                                    alt={val.location} 
                                    className='w-20 h-20 sm:w-16 sm:h-16 rounded-lg filter drop-shadow-lg'/>
                                </div>
                                <div className='flex items-start flex-col text-slate-900 hover:bg-emerald-300 transition-all duration-300 cursor-pointer hover:scale-105'>
